<x-app-layout>
    <div class="grid grid-cols-3 gap-8">
        ...
        <div class="col-span-2">
            <div class="bg-white/[0.03] rounded-2xl border border-white/10 shadow-2xl overflow-hidden">

                {{-- Form Header --}}
                <div class="bg-gradient-to-r from-white/5 to-white/10 border-b border-white/10 px-8 py-6">
                    <div class="flex items-center justify-between">
                        <div>
                            <h3 class="text-xl font-semibold text-white">Upload File Excel</h3>
                            <p class="text-slate-400 text-sm mt-1">Pilih file Excel yang berisi data mahasiswa</p>
                        </div>
                    ...
                    <form action="{{ route('admin.mahasiswa.import') }}" method="POST" enctype="multipart/form-data"
                        ...
                        <div class="space-y-6">
                            <div class="flex items-center space-x-3 mb-6">
                                ...
                                <h4 class="text-lg font-medium text-white">Pilih File Excel</h4>
                            </div>
                            ...
                            <div class="relative">
                                ...
                                <div id="file-drop-area"
                                    class="border-2 border-dashed border-white/30 rounded-2xl p-12 text-center hover:border-emerald-400/50 hover:bg-white/5 transition-all duration-300">
                                    <div class="space-y-4">
                                        <div class="flex justify-center">
                                            <div class="p-4 bg-emerald-500/20 rounded-2xl">
                                                <svg class="w-12 h-12 text-emerald-300" fill="none"
                                        ...
                                        <div>
                                            ...
                                            <p class="text-slate-400 text-sm">File Excel (.xlsx) maksimal 50MB</p>
                                        </div>
                                        <div id="file-info"
                                            class="hidden mt-4 p-4 bg-white/10 rounded-xl border border-white/20">
                                            <div class="flex items-center space-x-3">
                                                <svg class="w-6 h-6 text-emerald-300 flex-shrink-0" fill="none"
                                                    stroke="currentColor" viewBox="0 0 24 24">
                                                    <path stroke-linecap="round" stroke-linejoin="round"
                                                        stroke-width="2"
                                                        d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
                                                ...
                                                <div class="flex-grow">
                                                    <p id="file-name" class="text-white font-medium truncate text-left">
                                                    </p>
                                                    <p id="file-size" class="text-slate-400 text-sm text-left"></p>
                                                </div>
                                                <button type="button"
                                                    class="ml-2 text-slate-400 hover:text-emerald-400 focus:outline-none"
                            ...
                            <x-input-error :messages="$errors->get('file')" class="text-red-400 text-sm" />
                        </div>
                        ...
                        <div class="flex items-center justify-between pt-6 border-t border-white/10">
                            ...
                            <div class="flex items-center space-x-4">
                                <a href="{{ route('admin.mahasiswa.index') }}"
                                    class="px-6 py-3 bg-white/10 hover:bg-white/20 border border-white/20 rounded-xl text-slate-300 hover:text-white transition-all duration-200 font-medium">
                                    Batal
                                </a>
                                <button type="submit"
        ...
        <div class="space-y-6">
            {{-- Template Download --}}
            <div class="bg-white/[0.03] rounded-2xl border border-white/10 shadow-xl p-6">
                <div class="flex items-center space-x-3 mb-4">
                    ...
                    <h4 class="text-lg font-medium text-white">Template Excel</h4>
                </div>
                <p class="text-slate-400 text-sm mb-4">Download template untuk memastikan format yang benar</p>
                <a href="{{ asset('template_mahasiswa.xlsx') }}" download
                    class="inline-flex items-center space-x-2 px-4 py-2 bg-blue-500/20 hover:bg-blue-500/30 border border-blue-400/30 rounded-xl text-blue-300 hover:text-blue-200 transition-all duration-200 text-sm font-medium">
                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                            d="M12 10v6m0 0l-3-3m3 3l3-3m2 8H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z">
                        </path>
            ...
            <div class="bg-white/[0.03] rounded-2xl border border-white/10 shadow-xl p-6">
                <div class="flex items-center space-x-3 mb-4">
                    ...
                    <h4 class="text-lg font-medium text-white">Petunjuk Import</h4>
                </div>
                <div class="space-y-3 text-sm">
                    <div class="flex items-start space-x-3">
                        <div
                            class="flex-shrink-0 w-6 h-6 bg-white/10 rounded-full flex items-center justify-center text-xs text-white font-medium mt-0.5">
                            1</div>
                        <p class="text-slate-300">Download template Excel yang disediakan</p>
                    </div>
                    <div class="flex items-start space-x-3">
                        <div
                            class="flex-shrink-0 w-6 h-6 bg-white/10 rounded-full flex items-center justify-center text-xs text-white font-medium mt-0.5">
                            2</div>
                        <p class="text-slate-300">Isi data mahasiswa sesuai format template</p>
                    </div>
                    <div class="flex items-start space-x-3">
                        <div
                            class="flex-shrink-0 w-6 h-6 bg-white/10 rounded-full flex items-center justify-center text-xs text-white font-medium mt-0.5">
                            3</div>
                        <p class="text-slate-300">Pastikan tidak ada kolom yang kosong</p>
                    </div>
                    <div class="flex items-start space-x-3">
                        <div
                            class="flex-shrink-0 w-6 h-6 bg-white/10 rounded-full flex items-center justify-center text-xs text-white font-medium mt-0.5">
                            4</div>
                        <p class="text-slate-300">Upload file dan tunggu proses import selesai</p>
                    </div>
                </div>
            </div>
            ...
            <div class="bg-white/[0.03] rounded-2xl border border-white/10 shadow-xl p-6">
                <div class="flex items-center space-x-3 mb-4">
                    ...
                    <h4 class="text-lg font-medium text-white">Persyaratan</h4>
                </div>
                <div class="space-y-2 text-sm">
                    <div class="flex items-center space-x-2">
                        <div class="w-1.5 h-1.5 bg-red-400 rounded-full"></div>
                        <p class="text-slate-300">File harus berformat .xlsx</p>
                    </div>
                    <div class="flex items-center space-x-2">
                        <div class="w-1.5 h-1.5 bg-red-400 rounded-full"></div>
                        <p class="text-slate-300">Maksimal ukuran file 50MB</p>
                    </div>
                    <div class="flex items-center space-x-2">
                        <div class="w-1.5 h-1.5 bg-red-400 rounded-full"></div>
                        <p class="text-slate-300">Email harus unique</p>
                    </div>
                    <div class="flex items-center space-x-2">
                        <div class="w-1.5 h-1.5 bg-red-400 rounded-full"></div>
                        <p class="text-slate-300">NIM harus unique</p>
                    </div>
                </div>
            </div>
        </div>
