<x-app-layout>
    ...
    <div class="mb-8 bg-white/[0.03] border border-white/10 rounded-2xl p-6 shadow-lg">
        <div class="flex items-center gap-4">
            <div>
                <h1 class="text-3xl font-bold text-white mb-2">Bimbingan</h1>
                <p class="text-blue-100 text-sm">Ajukan sesi bimbingan dengan dosen pembimbing dan pantau status
                    pengajuan Anda</p>
            </div>
        </div>
        <div class="mt-4">
    ...
    <div class="mb-8 backdrop-blur-sm bg-white/[0.03] border border-white/10 rounded-2xl shadow-xl overflow-hidden">
        ...
        <div class="px-6 py-4 border-b border-white/10">
            <div class="flex items-center gap-3">
                ...
                <div>
                    <h3 class="text-lg font-semibold text-white">Form Pengajuan Bimbingan</h3>
                    <p class="text-sm text-slate-400">Lengkapi informasi di bawah untuk mengajukan sesi bimbingan</p>
                </div>
            </div>
        </div>
        ...
        <div class="p-6">
            <form action="{{ route('mahasiswa.bimbingan.store') }}" method="POST" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {{-- Pilih Dosen --}}
                    <div class="space-y-2">
                        <label for="dosen_id" class="block text-sm font-medium text-white/90">
                            Dosen Pembimbing
                        ...
                        <div class="relative">
                            <select name="dosen_id" id="dosen_id" required
                                ...
                                <option value="" class="bg-slate-800 text-slate-300">Pilih Dosen Pembimbing</option>
                                @foreach(App\Models\Dosen::all() as $dosen)
                                <option value="{{ $dosen->id }}" class="bg-slate-800 text-white">
                    ...
                    {{-- Tanggal Bimbingan --}}
                    <div class="space-y-2">
                        <label for="tanggal_bimbingan" class="block text-sm font-medium text-white/90">
                            Tanggal Bimbingan
                            <span class="text-red-400">*</span>
                        </label>
                        <input type="date" name="tanggal_bimbingan" id="tanggal_bimbingan" required
                            class="w-full px-4 py-3 bg-white/[0.05] text-white rounded-lg border border-white/20 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-400/50 focus:outline-none transition-all duration-200 hover:bg-white/10 hover:border-white/30 backdrop-blur-sm"
                ...
                {{-- Catatan --}}
                <div class="space-y-2">
                    <label for="catatan" class="block text-sm font-medium text-white/90">
                        Catatan Tambahan
                    ...
                    <textarea name="catatan" id="catatan" rows="4"
                        ...
                        class="w-full px-4 py-3 bg-white/[0.05] text-white rounded-lg border border-white/20 focus:ring-2 focus:ring-blue-500/50 focus:border-blue-400/50 focus:outline-none transition-all duration-200 hover:bg-white/10 hover:border-white/30 backdrop-blur-sm placeholder-slate-400 resize-none"></textarea>
                </div>
    ...
    {{-- Riwayat Bimbingan --}}
    <div class="backdrop-blur-sm bg-white/[0.03] border border-white/10 rounded-2xl shadow-xl overflow-hidden">
        {{-- Card Header --}}
        <div class="px-6 py-4 border-b border-white/10">
            <div class="flex items-center gap-3">
                <div
                ...
                <div>
                    <h3 class="text-lg font-semibold text-white">Riwayat Pengajuan Bimbingan</h3>
                    <p class="text-sm text-slate-400">Daftar semua pengajuan bimbingan yang pernah Anda ajukan</p>
                </div>
            </div>
        </div>
        ...
        <div class="overflow-x-auto">
            ...
            <div
                class="hidden md:grid md:grid-cols-12 gap-4 px-6 py-3 bg-white/[0.02] border-b border-white/10 text-xs font-semibold text-slate-300 uppercase tracking-wider">
            ...
            {{-- Table Row / Card Item --}}
            <div
                class="group hover:bg-white/[0.02] transition-all duration-200 border-b border-white/5 last:border-b-0">
                {{-- Desktop View --}}
                <div class="hidden md:grid md:grid-cols-12 gap-4 items-center px-6 py-4">
                    {{-- Number --}}
                    <div class="col-span-1">
                        <span
                            class="inline-flex items-center justify-center w-8 h-8 bg-slate-700/50 text-slate-300 text-sm font-medium rounded-lg group-hover:bg-slate-600/50 transition-colors">
                            {{ $i + 1 }}
                        </span>
                    </div>
                    ...
                    <div class="col-span-4 flex items-center gap-3">
                        ...
                        <div>
                            <p class="text-white font-medium">{{ $item->dosen->nama }}</p>
                            <p class="text-xs text-slate-400">{{ $item->dosen->program_studi }}</p>
                        </div>
                    </div>
                    ...
                    <div class="col-span-2 text-center">
                        ...
                        <span
                            class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-white/5 text-green-400 border border-green-500/30">
                        ...
                        <span
                            class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-white/5 text-red-400 border border-red-500/30">
                        ...
                        @else
                        <span
                            class="inline-flex items-center gap-2 px-4 py-2 text-sm font-semibold rounded-lg bg-white/5 text-yellow-400 border border-yellow-500/30">
            ...
            <div class="px-6 py-12 text-center">
                ...
                <h3 class="text-lg font-medium text-slate-300 mb-2">Belum Ada Pengajuan Bimbingan</h3>
        ...
        <div class="px-6 py-4 bg-white/[0.02] border-t border-white/10">
